#!/usr/bin/env python
"""
Phase 2 pipeline smoke test for LatentVANS.
...
NOT the native-format repo phase1_data_staging.sh already downloaded
...
Run phase1b_download_wan_diffusers.sh first to fetch the diffusers-format
checkpoint as an addition (it does not touch anything phase1 already staged).
...
Usage
-----
    python phase2_pipeline_smoke_test.py \\
...
def parse_args():
    ...
    p.add_argument("--wan_diffusers_dir", required=True,
                   help="Local path to the DIFFUSERS-format Wan2.1-T2V-1.3B-Diffusers checkpoint "
                        "(see phase1b_download_wan_diffusers.sh -- this is NOT the same directory "
                        "phase1_data_staging.sh downloaded)")
...
def main():
    ...
    if args.device == "cuda" and not cuda_ok:
        log("FATAL: --device cuda was requested but CUDA is not available here. If this is the "
            ...
            "(sbatch phase2_pipeline_smoke_test.sh, or "
    ...
    try:
        ...
        log(f"FATAL: missing dependency ({e}). Need transformers>=4.49.0 (Qwen2.5-VL support), "
            f"diffusers>=0.32.0 (Wan support), and torchvision (transformers' Qwen2-VL video "
            f"processor backend -- easy to miss since nothing else here imports it directly). Try:\n"
            f"  python -m pip install --upgrade 'transformers>=4.49.0' 'diffusers>=0.32.0' accelerate torchvision")
        sys.exit(1)
